...
export const LoansModule = () => {
  ...
  // Balance pendiente de un préstamo, para la tarjeta del listado.
  //
  // DELEGA EN `getLoanBalanceBreakdown`, la misma función que usan Detalles y el estado de
  // cuenta. Antes solo lo hacía para préstamos INDEFINIDOS, y para plazo fijo mantenía aquí
  // una copia de ~290 líneas de la misma lógica financiera.
  //
  // Esa copia arrastraba un fallo ya corregido en la función compartida: agrupaba lo pagado
  // por fecha de vencimiento SIN separar los pagos de cargos, así que al cobrar un cargo
  // fechado el mismo día que una cuota —lo normal— contaba ese dinero también como si hubiera
  // saldado la cuota. Un cargo de 3,000 bajaba el saldo 3,836.11: el cargo más una cuota
  // entera que nadie pagó.
  //
  // Se elimina la copia en vez de repararla: dos implementaciones de la misma cuenta acaban
  // divergiendo siempre, y de hecho ya habían divergido — la tarjeta decía 9,197.13 y la
  // tabla de cuotas 10,033.33 para el mismo préstamo, el mismo día.
